...
from .conversation import parse_utc_timestamp, format_utc_timestamp
...
class SummaryStore:
    ...
    def update_summary_metadata(
        ...
        descendant_token_count: Optional[int] = None,
        earliest_at: Optional[datetime] = None,
        latest_at: Optional[datetime] = None
    ):
    ...
    def get_context_items(self, conversation_id: int) -> List[ContextItemRecord]:
        ...
        items = []
        for row in cursor.fetchall():
            items.append(ContextItemRecord(
                id=row[0],
                conversation_id=row[1],
                position=row[2],
                item_type=row[3],
    ...
    def delete_summary(self, summary_id: str):
        ...
        with self.db.transaction() as conn:
            ...
            conn.execute("DELETE FROM summary_parents WHERE parent_id = ? OR child_id = ?", 
                        (summary_id, summary_id))
