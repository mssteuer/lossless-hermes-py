...
from .store.conversation import ConversationStore, MessageSearchInput, MessageSearchResult
...
class SearchResult:
    ...
    id: Union[int, str]  # message_id or summary_id
    conversation_id: int
    content: str
    snippet: str
...
class RetrievalEngine:
    ...
    def get_full_content(self, result: SearchResult) -> Optional[str]:
        """Get the full content for a search result."""
        
        if result.type == "message":
            messages = self.conversation_store.get_messages_by_conversation(
    ...
    def find_related_messages(
        ...
        filtered_results = [
            r for r in results 
            if not (r.type == "message" and r.id == message_id)
        ]
