...
from .retrieval import RetrievalEngine, SearchQuery
    ...
    return [
        {
            "name": "lcm_grep",
            "description": (
                "Search through conversation history and summaries using full-text search or"
                " regex patterns. Useful for finding past discussions, decisions, or specific"
                " technical details."
            ),
            "parameters": {
                "type": "object",
        ...
        },
        {
            "name": "lcm_describe",
            "description": (
                "Get detailed information about the current conversation's LCM state,"
                " including summary statistics, compaction history, and DAG structure."
            ),
            "parameters": {
                "type": "object",
                "properties": {
        ...
        },
        {
            "name": "lcm_expand",
            "description": (
                "Retrieve and expand specific content from the conversation history."
                " Can get full message content, summary details, or related context."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "target_type": {
                        ...
                        "description": (
                            "Type of content to expand: 'message' for full message content,"
                            " 'summary' for summary details, 'related' for finding related content."
                        ),
                    },
                    "target_id": {
                        "type": "string",
...
class LcmTools:
    ...
    def _handle_lcm_describe(self, args: dict[str, Any]) -> str:
        ...
        if include_recent:
            ...
            recent_summaries = self.summary_store.get_summaries_by_conversation(self.conversation_id)
    ...
    def _handle_lcm_expand(self, args: dict[str, Any]) -> str:
        ...
        if target_type == "message":
            if not target_id:
                return json.dumps({"error": "target_id is required for message expansion"})

            try:
                message_id = int(target_id)
            except (ValueError, TypeError):
                return json.dumps({"error": "target_id must be a valid message ID"})
            ...
            if not target_id:
                return json.dumps({"error": "target_id is required for summary expansion"})

            summary = self.summary_store.get_summary(target_id)
            if not summary:
                return json.dumps({"error": f"Summary {target_id} not found"})
            ...
            if not query:
                return json.dumps({"error": "query is required for related content search"})
            ...
            similar_conversations = self.retrieval_engine.search_similar_conversations(
                query=query, exclude_conversation_id=self.conversation_id, limit=5
            )
