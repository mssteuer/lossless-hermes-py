...
from .db.config import LcmConfig, CacheAwareCompactionConfig
from .tokens import estimate_tokens, estimate_messages_tokens
from .summarizer import SyncLcmSummarizer, SummaryOptions
from .store.conversation import ConversationStore, MessageRecord
from .store.summary import SummaryStore, SummaryRecord, CreateSummaryInput
...
class CompactionEngine:
    ...
    def should_compact(
        ...
        if self.config.cache_aware_compaction.enabled:
            return self._should_compact_cache_aware(
                conversation_id, current_tokens, threshold_tokens, cache_state
            )
    ...
    def _run_condensed_compaction(self, conversation_id: int, cache_state: str) -> CompactionResult:
        ...
        for depth in range(self.config.incremental_max_depth + 1):
            depth_summaries = self.summary_store.get_summaries_by_conversation(
                conversation_id, depth=depth
            )
            ...
            for chunk in chunks:
                ...
                if summary:
                    ...
                    for parent_summary in chunk:
                        self.summary_store.add_summary_parent(
                            summary.summary_id, parent_summary.summary_id
                        )
    ...
    def _chunk_messages_for_leaf_compaction(
        self, 
        ...
        for message in messages:
            ...
            if (current_tokens + message.token_count > self.config.leaf_chunk_tokens 
    ...
    def _chunk_summaries_for_condensed_compaction(
        self,
        ...
        for summary in summaries:
            if (current_tokens + summary.token_count > target_tokens 
    ...
    def _create_leaf_summary(
        ...
        try:
            ...
            return self.summary_store.create_summary(CreateSummaryInput(
